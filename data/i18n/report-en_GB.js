export default {
    "report_about_this_report": "About this report",
    "report_about_this_report_default_text_product_development": "This report is the result of a careful integration of the results of all assessment components by the responsible consultant. Based on this the consultant has written a personal report consisting of an advice, scores and explanation. The report is intended to support gaining insight into the strengths, development points and development opportunities of the participant.",
    "report_about_this_report_default_text_product_selection": "This report is the result of a careful integration of the results of all assessment components by the responsible consultant. Based on this the consultant has written a personal report consisting of an advice, scores and explanation. The report is intended to support decision-making on placement and gaining insight into the strengths and development points of the participant.",
    "report_about_this_report_default_text_product_selection_development": "This report is the result of a careful integration of the results of all assessment components by the responsible consultant. Based on this the consultant has written a personal report consisting of an advice, scores and explanation. The report is intended to support decision-making on placement and gaining insight into the strengths, development points and development opportunities of the participant.",
    "report_abstract_intelligence": "Abstract intelligence",
    "report_abstract_intelligence_test": "Abstract intelligence test",
    "report_abstract_intelligence_text": "This score gives an indication of the extent to which someone can think in the abstract, knows how to separate main and side issues, know how to penetrate to the core of a problem, can stand above a problem and can see things in a broader context.",
    "report_altruism": "Altruism",
    "report_anticipating": "Anticipating",
    "report_anticipating_high_text": "<ul>\n                            <li>Always thinks three steps ahead before undertaking something.<\/li>\n                            <li>Anticipates possible obstacles so that his\/her own objectives can be realised.<\/li>\n                        <\/ul>",
    "report_anticipating_low_text": "<ul>\n                            <li>Does not always think things through before taking action.<\/li>\n                            <li>Sometimes acts on intuition or impulse. He\/she therefore regularly makes decisions that are wrong, faces situations that he\/she failed to anticipate or elicits unexpected reactions from other people.<\/li>\n                        <\/ul>",
    "report_april": "April",
    "report_assertive": "Assertive",
    "report_assertive_high_text": "<ul>\n                            <li>Is someone who fights for his\/her own interests.<\/li>\n                            <li>Will not be easily convinced to change his\/her mind.<\/li>\n                            <li>The assertiveness with which he\/she defends his\/her own opinions and interests gives him\/her the upper hand in work situations.<\/li>\n                        <\/ul>",
    "report_assertive_low_text": "<ul>\n                            <li>Often has difficulty clearly stating his\/her own goals and priorities.<\/li>\n                            <li>Often hesitates; others can easily influence him\/her. An inability to state what he\/she wants can be an inhibiting factor in work situations.<\/li>\n                        <\/ul>",
    "report_assessment_date": "Assessment date",
    "report_attachment_grade_default_1": "Far below average",
    "report_attachment_grade_default_2": "Below average",
    "report_attachment_grade_default_3": "Average",
    "report_attachment_grade_default_4": "Above average",
    "report_attachment_grade_default_5": "High",
    "report_attachment_grade_intelligence_1": "Far below average",
    "report_attachment_grade_intelligence_2": "Below average",
    "report_attachment_grade_intelligence_3": "Average",
    "report_attachment_grade_intelligence_4": "Above average",
    "report_attachment_grade_intelligence_5": "Well above average",
    "report_august": "August",
    "report_competency_profile": "Competency profile",
    "report_components": "Components",
    "report_components_default_text": "The assessment report is based on the results of the following components:\n\n                            <ul>\n                                <li>Personality questionnaires<\/li>\n                                <li>Capacities tests<\/li>\n                                <li>Interview<\/li>\n                                <li>Simulation<\/li>\n                            <\/ul>",
    "report_conformity": "Conformity",
    "report_conscientiousness": "Conscientiousness",
    "report_conscientiousness_high_text": "<ul>\n                            <li>Is orderly, methodical and has a sense of responsibility.<\/li>\n                            <li>Pays attention to details.<\/li>\n                            <li>Can concentrate well on tasks and objectives, and is disciplined in finishing matters.<\/li>\n                            <li>Finds rules, preparation and the fulfilment of agreements important.<\/li>\n                            <li>Can be too precise or strict in complying with rules and procedures; is therefore in the eyes of others less flexible.<\/li>\n                        <\/ul>",
    "report_conscientiousness_low_text": "<ul>\n                            <li>Emphasizes general lines, allows him\/herself to be guided by what is happening at the moment. Is less concerned with details.<\/li>\n                            <li>Is flexible and able to adapt behaviour and plans to the situation. Has no trouble breaking off a task or to conducting several projects simultaneously.<\/li>\n                            <li>Can be viewed by others as chaotic and careless; in their eyes he\/she is less systematic.<\/li>\n                            <li>Offers others little structure and clarity.<\/li>\n                        <\/ul>",
    "report_consultant": "Consultant",
    "report_december": "December",
    "report_decisiveness": "Decisiveness",
    "report_decisiveness_high_text": "<ul>\n                            <li>Is not hesitant and gets on with things.<\/li>\n                            <li>Will make a decision based on the available information when choices need to be made.<\/li>\n                            <li>Prefers to solve problems immediately.<\/li>\n                        <\/ul>",
    "report_decisiveness_low_text": "<ul>\n                            <li>Is often hesitant and has difficulty getting on with things.<\/li>\n                            <li>Finds it difficult to make decisions when choices need to be made.<\/li>\n                        <\/ul>",
    "report_development_advice": "Development advice",
    "report_development_orientation": "Development oriented",
    "report_development_orientation_high_text": "<ul>\n                            <li>Is willing to make efforts to acquire new knowledge and skills by means of courses or independent study.<\/li>\n                            <li>This is not only in order to do the work more effectively but also for their his\/her personal development.<\/li>\n                        <\/ul>",
    "report_development_orientation_low_text": "<ul>\n                            <li>Considers acquiring new skills and knowledge through study not to be the most important aspect of work.<\/li>\n                            <li>Professional development is not an active area of focus.<\/li>\n                        <\/ul>",
    "report_development_oriented": "Development oriented",
    "report_development_oriented_default_text": "The belief that it is possible, useful and pleasant to work on your own development (‘growth mindset’).",
    "report_development_report": "Development report",
    "report_download_pdf_current_version": "Current version",
    "report_download_pdf_explanation": "A new PDF is generated. This may take a while.\nThe report can be downloaded once the report is generated.",
    "report_download_pdf_generate": "Generate report",
    "report_download_pdf_problem_downloading": "An error occurred while downloading the report. Please try again later.",
    "report_download_pdf_problem_generating": "There was a problem generating the report. Please try again later.",
    "report_download_pdf_title": "Report as pdf",
    "report_emotional_stability": "Emotional stability",
    "report_emotional_stability_high_text": "<ul>\n                            <li>Is balanced and deals with stressful situations in a controlled manner.<\/li>\n                            <li>Feels self-assured and has the idea that he\/she has situations under control.<\/li>\n                            <li>Looks at problems rationally. sees primarily the positive side of matters, can therefore underestimate problems.<\/li>\n                            <li>Can be perceived by others as being (too) little sensitive.<\/li>\n                        <\/ul>",
    "report_emotional_stability_low_text": "<ul>\n                            <li>Is sensitive, has an eye for and grasp of the feelings of others. is good at empathising.<\/li>\n                            <li>Allows his\/her behaviour to be guided by feeling.<\/li>\n                            <li>Is sensitive to atmosphere or tension when under pressure, can then have an above-average emotional reaction.<\/li>\n                            <li>Is alert in the event of possible problems, is not quick to underestimate them.<\/li>\n                        <\/ul>",
    "report_energy": "Energy",
    "report_energy_default_text": "Being inquisitive and playful. Actively seek out new learning opportunities.",
    "report_error_download_pdf": "An error occurred while downloading the pdf report. Please try again.",
    "report_error_save_score": "An error occurred while saving. Please try again.",
    "report_error_save_text": "An error occurred while saving. Please try again.",
    "report_explanation": "Explanation",
    "report_extraversion": "Extraversion",
    "report_extraversion_high_text": "<ul>\n                            <li>Is talkative, lively and active.<\/li>\n                            <li>Is glad to be in company, derives energy from social contact.<\/li>\n                            <li>Enjoys being where the action is.<\/li>\n                            <li>Easily assumes the centre-stage in company, can dominate the situation and be found too boisterous.<\/li>\n                            <li>Takes the initiative in making contacts.<\/li>\n                        <\/ul>",
    "report_extraversion_low_text": "<ul>\n                            <li>Is calm, unto him\/herself and retiring.<\/li>\n                            <li>Is glad to work on assignments on his\/her own and independently.<\/li>\n                            <li>Prefers a calm environment, with few stimulants.<\/li>\n                            <li>Keeps in the background in company.<\/li>\n                            <li>Allows others space.<\/li>\n                            <li>Is held-back in making contacts.<\/li>\n                        <\/ul>",
    "report_february": "February",
    "report_fit": "FIT",
    "report_fit_match": "FIT To be\/To want",
    "report_function": "Function",
    "report_goal_oriented": "Goal-oriented",
    "report_goal_oriented_high_text": "<ul>\n                            <li>Usually has a clear goal in mind.<\/li>\n                            <li>Thinks ahead and strives hard to achieve the goal.<\/li>\n                        <\/ul>",
    "report_goal_oriented_low_text": "<ul>\n                            <li>Usually does not have a clearly defined goal in mind.<\/li>\n                            <li>Generally takes things as they come, rather than deciding on a direction in advance.<\/li>\n                        <\/ul>",
    "report_high": "High",
    "report_high_score": "High score",
    "report_independence": "Independence",
    "report_influencing_styles": "Influencing styles",
    "report_influencing_styles_default_text": "<p>In order to achieve personal goals, one often needs other people. In general, everyone has his or her own way of influencing other people in order to achieve certain personal goals. Below, you can see the extent to which the participant applies the different influencing styles.<\/p><p>The following section of the report is automated and solely based on the results of the personality questionnaire. The other components of the assessment are not included.<\/p>",
    "report_intellectual_capabilities": "Intellectual capabilities",
    "report_intellectual_capabilities_default_text_paragraph_1": "<p>You have completed a number of ability tests that measure different intellectual abilities. Here you will see the total score on intelectual ability.<\/p>\n<p>Your scores on these tests are set against the applicable norm group; the tested level.<\/p>\n<p>This refers to the thinking level required for the job function. This thinking level does not necessarily have to be related to your educational level.<\/p>",
    "report_intellectual_capabilities_default_text_paragraph_2": "<p>The results below are the results on the individual ability tests. Please note that the overall score that is described earlier in the report, is not an average of the tests listed below. Relationships between different tests ensure that the overall score may be higher or lower than the average score.<\/p><p>The following section of the report is automated and solely based on the results of the ability tests. The other components of the assessment are not included.<\/p>",
    "report_intelligence_score": "Intellectual capabilities",
    "report_introduction": "Introduction",
    "report_january": "January",
    "report_jobfunction": "Job function",
    "report_july": "July",
    "report_june": "June",
    "report_leadership": "Leadership",
    "report_looking_for_challenges": "Looking for challenges",
    "report_looking_for_challenges_high_text": "<ul>\n                            <li>Requires a deadline or similar form of tension in order to get down to work.<\/li>\n                            <li>Finds it difficult to start work on certain activities, without some form of pressure.<\/li>\n                        <\/ul>",
    "report_looking_for_challenges_low_text": "<ul>\n                            <li>Does not require a deadline or similar form of tension in order to get down to work.<\/li>\n                            <li>Is able to start work without too much encouragement.<\/li>\n                        <\/ul>",
    "report_low": "Low",
    "report_low_score": "Low score",
    "report_ltp_influencing_styles_test": "LTP Influencing styles",
    "report_ltp_motives_test": "LTP Motives",
    "report_ltp_personality_test": "LTP Personality",
    "report_ltp_working_styles_test": "LTP Working styles",
    "report_manipulative": "Manipulative",
    "report_manipulative_high_text": "<ul>\n                            <li>Is adept at manipulating people. Tries to achieve his\/her goals by whatever means required.<\/li>\n                            <li>Interactions with others tend to be shaped by the extent to which they can be of use to him\/her.<\/li>\n                        <\/ul>",
    "report_manipulative_low_text": "<ul>\n                            <li>Often opts for the safe, familiar approach when something needs to be achieved.<\/li>\n                            <li>Does not believe that any method is justified, however important the goal may be.<\/li>\n                            <li>Is generally not particularly adept at manipulating people.<\/li>\n                        <\/ul>",
    "report_march": "March",
    "report_may": "May",
    "report_mildness": "Mildness",
    "report_mildness_high_text": "<ul>\n                            <li>Is helpful and focussed on collaboration.<\/li>\n                            <li>Assumes the best about people, believes that others are of goodwill.<\/li>\n                            <li>Is more focussed on the welfare of others than on the business or his\/her own interest.<\/li>\n                            <li>Therefore can efface him\/herself.<\/li>\n                        <\/ul>",
    "report_mildness_low_text": "<ul>\n                            <li>Is business-like or distant in dealings with others.<\/li>\n                            <li>Does not accord his\/her trust just like that, keeps his\/her intentions to him\/herself.<\/li>\n                            <li>Is more focussed on the business than on the welfare of others.<\/li>\n                            <li>Has little trouble defending him\/herself and setting limits.<\/li>\n                        <\/ul>",
    "report_motives": "Motives",
    "report_motives_default_text": "<p>Motives can be regarded as the person’s drives. These are personal convictions, desires, preferences or even dislikes that prompt the person to seek, or indeed to avoid, something. Motives can be regarded as the driving force behind the behaviour of people in all kinds of situations. Motives are very personal. What might be an important motive for one person, does not need to be an important motive for somebody else. It is known that some motives depend heavily on the life-phase that the person is in. This applies, for example, to career motives. Other motives are more stable and remain more or less present over an entire life. With regard to the motives below, these are present in everyone to a greater or lesser extent. Here, too, variation is possible, depending on the life-phase, but everyone can be characterised on the basis of scores on these motives.<\/p><p>The following section of the report is automated and solely based on the results of the personality questionnaire. The other components of the assessment are not included.<\/p>",
    "report_need_for_acceptance": "Need for acceptance",
    "report_need_for_acceptance_high_text": "<ul>\n                            <li>Needs harmony in relationships. Takes account of others, considers other people's approval to be important, and wants to be seen as a nice person.<\/li>\n                            <li>Invests energy in developing and maintaining enjoyable, good working relationships with others. Can avoid conflict.<\/li>\n                        <\/ul>",
    "report_need_for_acceptance_low_text": "<ul>\n                            <li>Feels little need to invest in relationships or devote energy to developing enjoyable working relationships.<\/li>\n                            <li>Considers it not especially important to take account of others and gain approval. Does not consider it important to be seen as a nice person.<\/li>\n                        <\/ul>",
    "report_need_for_autonomy": "Need for autonomy",
    "report_need_for_autonomy_high_text": "<ul>\n                            <li>Needs freedom and independence. Wants to have the freedom to act according to their his\/her views and ideas.<\/li>\n                            <li>Has little interest in rules, standards or agreements. Considers it important to have as few obligations to others as possible.<\/li>\n                        <\/ul>",
    "report_need_for_autonomy_low_text": "<ul>\n                            <li>Has little need for freedom and autonomy. Accepts rules, agreements and working methods or guidelines imposed by others.<\/li>\n                            <li>Considers it less important to have the freedom to organise the work according to his\/her own insights.<\/li>\n                        <\/ul>",
    "report_need_to_influence": "Need to influence",
    "report_need_to_influence_high_text": "<ul>\n                            <li>Has a strong need to exercise influence and take the lead.<\/li>\n                            <li>Enjoys encouraging discussion and arguing. Does not avoid conflict.<\/li>\n                            <li>Has a tendency to see situations in terms of winning\/losing and to force his\/her own views through, making use of his\/her own position and influence.<\/li>\n                        <\/ul>",
    "report_need_to_influence_low_text": "<ul>\n                            <li>Has little need to exert influence and to make important decisions.<\/li>\n                            <li>Likes to listen to others during discussions and to leave the important decision-making to others.<\/li>\n                            <li>Does not find it important to engage in discussions or to make use of his\/her position to create support for his\/her own points of view.<\/li>\n                        <\/ul>",
    "report_negative": "Negative",
    "report_november": "November",
    "report_numeric_intelligence": "Numeric intelligence",
    "report_numeric_intelligence_test": "Numeric intelligence test",
    "report_numeric_intelligence_text": "This score refers to the skill in working with and interpreting numerical information. Amongst other things, this skill is important in positions in which one is expected to substantiate matters with numbers.",
    "report_october": "October",
    "report_open_mindness": "Open-mindness",
    "report_open_mindness_high_text": "<ul>\n                            <li>Is creative and original in coming up with new ideas or solutions.<\/li>\n                            <li>Is critical and has his\/her own judgement which is independent of what others assert.<\/li>\n                            <li>Is always in search of something new.<\/li>\n                            <li>Can be considered less practical and concrete.<\/li>\n                        <\/ul>",
    "report_open_mindness_low_text": "<ul>\n                            <li>Has a practical and concrete attitude and wants to solve problems as quickly as possible.<\/li>\n                            <li>Tends to follow accepted paths. Opts for a tried and tested approach in the solution of problems, and is not quick to try out new things.<\/li>\n                            <li>Easily follows the majority in what they say and how they want to tackle matters.<\/li>\n                        <\/ul>",
    "report_organisation": "Organisation",
    "report_page_title": "Report",
    "report_participant": "Participant",
    "report_performance_motivation": "Performance motivation",
    "report_performance_motivation_high_text": "<ul>\n                            <li>Has a strong need to take on challenging activities and to outperform others.<\/li>\n                            <li>Enjoys activities that can exceed expectations and for which the results can clearly be traced to the individual involved. Is eager to achieve something.<\/li>\n                            <li>Is not so inclined to invest in activities where little is expected of the results.<\/li>\n                        <\/ul>",
    "report_performance_motivation_low_text": "<ul>\n                            <li>Has little need to distinguish him\/herself and to outperform others.<\/li>\n                            <li>Does not find it important that results can be clearly traced back to the individual, in order to make his\/her performance visible.<\/li>\n                            <li>Does not really have a need to achieve or to build something.<\/li>\n                        <\/ul>",
    "report_performance_uncertainty": "Performance uncertainty",
    "report_performance_uncertainty_high_text": "<ul>\n                            <li>Can often feel insecure when unfamiliar, complex activities need to be undertaken.<\/li>\n                            <li>Can become inhibited by the thought that things may go wrong, in such situations.<\/li>\n                        <\/ul>",
    "report_performance_uncertainty_low_text": "<ul>\n                            <li>Is generally confident when approaching new jobs or activities.<\/li>\n                            <li>Does not tend to worry that something could go wrong in such cases.<\/li>\n                        <\/ul>",
    "report_personality": "Personality",
    "report_personality_default_text": "<p>Personality factors are a person’s stable characteristics. This means that with the passage of time they do not, or hardly, change. Personality characteristics have a strong influence on behaviour, a person’s style and the skills that someone develops through life. Innumerable studies, conducted throughout the world, have led to the insight that someone’s personality can best be typified on the basis of the following five stable personality characteristics. your personality profile is displayed below.<\/p><p>The following section of the report is automated and solely based on the results of the personality questionnaire. The other components of the assessment are not included.<\/p>",
    "report_points_of_attention_and_risks": "Points of attention and risks",
    "report_positive": "Positive",
    "report_power_to_change": "Power to change",
    "report_power_to_change_above_average": "Above average",
    "report_power_to_change_below_average": "Below average",
    "report_power_to_change_default_text_paragraph_1": "<p>The score on power to change indicates to what degree someone has the attitude and motivation to (continue to) function well when changes occur at work.<\/p>\n<p>Determining factors in this context are resilience, energy, selfmanagement and being development-oriented.<\/p>",
    "report_power_to_change_score": "Power to change",
    "report_product_development": "Development report",
    "report_product_selection": "Selection report",
    "report_product_selection_development": "Development report with selection advice",
    "report_project": "Project",
    "report_qualities_and_opportunities": "Qualities and opportunities",
    "report_quality_achievement": "Quality achievement",
    "report_quality_achievement_high_text": "<ul>\n                            <li>Delivers work that meets the own (high) quality standards.<\/li>\n                            <li>Will make every effort to bring them to a successful conclusion, even if certain activities are felt to be not especially enjoyable.\n                            <\/li>\n                        <\/ul>",
    "report_quality_achievement_low_text": "<ul>\n                            <li>Work that meets the organisation's general standards is good enough.<\/li>\n                            <li>Is not especially driven by a personal need to deliver work of the highest quality.<\/li>\n                        <\/ul>",
    "report_questionnaire": "Questionnaire",
    "report_questionnaires": "Questionnaires",
    "report_report": "Report",
    "report_report_details": "Report details",
    "report_research_question": "Research question",
    "report_research_question_default_text_product_development": "<ul><li>What are the strengths and areas of improvement, with reference to the job function?<\/li>\n                                <li>What can be done to better exploit strengths and develop areas of improvement?<\/li><\/ul>",
    "report_research_question_default_text_product_selection": "<ul><li>What are the strengths and areas of improvement, with reference to the job function?<\/li>\n                            <li>Is it sufficiently likely that the participant will be successful in this job function?<\/li><\/ul>",
    "report_research_question_default_text_product_selection_development": "<ul><li>What are the strengths and areas of improvement, with reference to the job function?<\/li>\n                            <li>Is it sufficiently likely that the participant will be successful in this job function?<\/li>\n                            <li>What can be done to better exploit strengths and develop areas of improvement?<\/li><\/ul>",
    "report_resilience": "Resilience",
    "report_resilience_default_text": "Picking up the thread after setbacks.",
    "report_results_report_pagina": "Results",
    "report_retest_button": "Retest",
    "report_retest_failed": "The request for a retest is failed. Please try again.",
    "report_retest_success": "The request for a retest has successfully been received. This may take a while...",
    "report_retest_success_description": "The request for a retest has successfully been received. This may take a while. Please refresh the page later so see the results.",
    "report_retest_title": "Trigger retest",
    "report_score_the_following_competencies": "Score the following competencies",
    "report_selection_advice": "Selection advice",
    "report_selection_advice_default_text": "Based on the results our advice for this job function is:",
    "report_selection_development_report": "Development report with selection advice",
    "report_selection_report": "Selection report",
    "report_self_management": "Self management",
    "report_self_management_default_text": "Using reactions and insights from others to increase your self-knowledge and improve your performances.",
    "report_september": "September",
    "report_share_report": "Share report",
    "report_straightforward": "Straightforward",
    "report_straightforward_high_text": "<ul>\n                            <li>Believes that honesty and a direct approach are ultimately the most effective, whatever the circumstances.<\/li>\n                            <li>Refuses to waste time playing games or beating about the bush.<\/li>\n                            <li>May appear blunt, but when others get to know him\/her, they will realise it is generally well-intentioned.<\/li>\n                        <\/ul>",
    "report_straightforward_low_text": "<ul>\n                            <li>Is someone who finds it difficult to say things directly to people.<\/li>\n                            <li>Attempts to express his\/her opinion carefully and in a well-considered way.<\/li>\n                            <li>Others may appreciate this subtle approach, although a more direct, focused approach achieves more in certain situations.<\/li>\n                        <\/ul>",
    "report_structure": "Structure",
    "report_structure_of_the_report": "Structure of the report",
    "report_structure_of_the_report_default_text_product_development": "The following elements are included in this report:\n\n                            <ul>\n                               <li>Research question<\/li>\n                               <li>Competency profile<\/li>\n                               <li>Intellectual capabilities (IQ)<\/li>\n                               <li>Power to change (VQ)<\/li>\n                               <li>Qualities and opportunities<\/li>\n                               <li>Points of attention and risks<\/li>\n                               <li>Development advice<\/li>\n                               <li>Results from tests and questionnaires<\/li>\n                            <\/ul>",
    "report_structure_of_the_report_default_text_product_selection": "The following elements are included in this report:\n\n                            <ul>\n                                <li>Research question<\/li>\n                                <li>Selection advice<\/li>\n                                <li>Competency profile<\/li>\n                                <li>Intellectual capabilities (IQ)<\/li>\n                                <li>Power to change (VQ)<\/li>\n                                <li>Qualities and opportunities<\/li>\n                                <li>Points of attention and risks<\/li>\n                                <li>Results from tests and questionnaires<\/li>\n                            <\/ul>",
    "report_structure_of_the_report_default_text_product_selection_development": "The following elements are included in this report:\n\n                            <ul>\n                             <li>Research question<\/li>\n                             <li>Selection advice<\/li>\n                             <li>Competency profile<\/li>\n                             <li>Intellectual capabilities (IQ)<\/li>\n                             <li>Power to change (VQ)<\/li>\n                             <li>Qualities and opportunities<\/li>\n                             <li>Points of attention and risks<\/li>\n                             <li>Development advice<\/li>\n                             <li>Results from tests and questionnaires<\/li>\n                            <\/ul>",
    "report_success_achievement": "Success achievement",
    "report_success_achievement_high_text": "<ul>\n                            <li>Considers it important to show success to the outside world.<\/li>\n                            <li>Is eager to make it clear what the contribution was to particular activities.<\/li>\n                            <li>Strives to achieve status and prestige.<\/li>\n                        <\/ul>",
    "report_success_achievement_low_text": "<ul>\n                            <li>Is not especially concerned as to whether succes is obvious to the outside world.<\/li>\n                            <li>Does not especially need to make it clear what the contribution was to particular activities and is not especially concerned about status.<\/li>\n                        <\/ul>",
    "report_support_and_recognition": "Support and recognition",
    "report_taking_risks": "Taking risks",
    "report_taking_risks_high_text": "<ul>\n                            <li>Enjoys the excitement involved in more difficult and uncertain activities.<\/li>\n                            <li>Has the confidence to take risks if opportunities emerge.<\/li>\n                        <\/ul>",
    "report_taking_risks_low_text": "<ul>\n                            <li>Enjoys work that has a clearly-defined outcome.<\/li>\n                            <li>Prefers leaving it to others to try out new approaches that may involve risks.<\/li>\n                        <\/ul>",
    "report_tentative": "Suitable, under conditions",
    "report_tested_level": "Tested level:",
    "report_tested_level_high": "Master's degree.",
    "report_tested_level_low": "Middle manual vocational program.",
    "report_tested_level_middle": "Higher vocational program (Bachelor's degree).",
    "report_to_be": "To be",
    "report_to_be_connector": "To be - Connector",
    "report_to_be_controller": "To be - Controller",
    "report_to_be_coordinator": "To be - Coordinator",
    "report_to_be_entrepreneur": "To be - Entrepreneur",
    "report_to_be_expert": "To be - Expert",
    "report_to_be_go_getter": "To be - Go-Getter",
    "report_to_be_innovator": "To be - Innovator",
    "report_to_be_inspirator": "To be - Inspirator",
    "report_to_want": "To want",
    "report_to_want_connector": "To want - Connector",
    "report_to_want_controller": "To want - Controller",
    "report_to_want_coordinator": "To want - Coordinator",
    "report_to_want_entrepreneur": "To want - Entrepreneur",
    "report_to_want_expert": "To want - Expert",
    "report_to_want_go_getter": "To want - Go-Getter",
    "report_to_want_innovator": "To want - Innovator",
    "report_to_want_inspirator": "To want - Inspirator",
    "report_total_intelligence": "Total",
    "report_validity": "Validity",
    "report_validity_default_text": "People change over time. New experiences, changed circumstances – in professional and private life – can lead to significant changes. For this reason, the validity of a personal assessment report is limited. We accordingly recommend that you consider the report’s contents applicable for only a period of about two years.",
    "report_values_questionnaire": "Values Questionnaire",
    "report_variety": "Variety",
    "report_verbal_intelligence": "Verbal intelligence",
    "report_verbal_intelligence_test": "Verbal intelligence test",
    "report_verbal_intelligence_text": "This score refers to the ability to see logical relations between linguistic semantics. Amongst other things, this is relevant when trying to understand complex texts.",
    "report_working_styles": "Working styles",
    "report_working_styles_default_text": "<p>People differ in terms of the way in which they achieve performance at work. In which areas does someone really push him\/herself? And what factors make someone more or less likely to perform well in particular situations? The following results give insight into the work style of the participant.<\/p><p>The following section of the report is automated and solely based on the results of the personality questionnaire. The other components of the assessment are not included.<\/p>"
};